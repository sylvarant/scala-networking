...
abstract class VirusSimulation extends Application {
  ...
  name = "Turn-based Social Virus Simulation"
...
class TBSocialVirusSimulation extends VirusSimulation {
...
class RBSocialVirusSimulation extends VirusSimulation {
  name = "Round-based Social Virus Simulation"
...
class EBSocialVirusSimulation extends VirusSimulation{
  
  name = "Event-based Social Virus Simulation"
   ...
   */
  network.adam.setAttribute("infected",false);
    /** dispatch the event to the engine */
  (new InfectionEvent(network.adam,5)).send
  }
  /**
   * A simple Event that takes place at a point in time t, and that will infect the node
   */
  class InfectionEvent(n : Actor, t : Integer) extends networkframework.core.engine.Event(t) {
  ...
  def checkCondition(e : Event){
    e match{
      case StatisticXUpdatedEvent(stat,engine) => stat match {
        case LivingActors() => if(stat.value ==  5) 
        { 
          var infect = engine.graph.getNodes.take(3).foreach(n => n.setAttribute("infected",true));
        };
        case _ =>
      }
      case _ =>
